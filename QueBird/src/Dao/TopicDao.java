...
//	获取每一个分页的数据
	public List<TopicBean> getListByPage(int start,int size){
		String sql = "select top " + size + " * from topic where(id not in(select top " + start + " id from topic))";
		System.out.println(sql);
	...
	{
		String sql="select * from (select top 10 * from topic order by id) table1 order by id desc";
		System.out.println(sql);
